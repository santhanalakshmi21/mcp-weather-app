...
class SimpleWeatherClient {
  ...
  async getCurrentWeather(city: string, units: string = "metric"): Promise<string> {
    if (!this.apiKey) {
...
💨 Wind Speed: ${weatherData.windSpeed} ${speedUnit}`;
  ...
  async getWeatherForecast(city: string, units: string = "metric"): Promise<string> {
    if (!this.apiKey) {
      return "❌ Error: OpenWeather API key not configured. Please set OPENWEATHER_API_KEY in .env file";
    }
